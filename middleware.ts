...
export async function middleware(req: NextRequest) {
  ...
  try {
    ...
    const isDniRoute = nextUrl.pathname.startsWith("/dni"); // Routes DNI (publiques)
    const isPublicRoute = publicRoutes.includes(nextUrl.pathname) || isDniRoute; // Public routes
    ...
    if (isApiRoute) {
      ...
      // Les APIs DNI sont publiques (vérification des tickets)
      if (nextUrl.pathname.startsWith("/api/dni")) {
        return NextResponse.next();
      }
